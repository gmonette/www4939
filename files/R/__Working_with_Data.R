...
#'   want to read the second worksheet that uses 'NA' for missing values, use:<br>
#'   `dd <- read_excel('file.xlsx', sheet = 2, na = 'NA')`
#' - If you want to read an Excel file on a web server (e.g. blackwell), some functions
#'   that read files, e.g. `read.csv`, will accept the URL instead of a path to a
#'   local file.  However, at this time, `read_excel` requires a local path. Thus,
#'   you need to download the file before reading it with `read_excel`. The usual way
#'   to download a file within R uses the `download.file` function. However, the default
#'   way to download
#'   binary Excel files, may corrupt the file. Try using the 'curl' method as illustrated
#'   below. This may not be necessary on Macs. Please let us know on Piazza!
#' - There are two xlsx files on blackwell:
#'     - 'file.xlsx': a small Excel file with clean data except that a numerical 
#'       value was entered as '$1,000.00'
#'     - 'file2.xlsx': same as above except that there is an 'invisible' single blank
#'       in the first column of the row after the actual data.  `read_excel` will interpret
#'       this as an NA by default and create an entire row of NA values.  Also, some entries are
#'       blank and some entries have been indicated as NAs by entering 'NA'. These common irregularities
#'       in Excel files can create havoc unless you are ready to deal with them. 
#'       
...
download.file(paste0(dir,'file.xlsx'),'_file.xlsx', 
              method = 'curl') # download to _file.xlsx to avoid overwriting 
download.file(paste0(dir,'file2.xlsx'),'_file2.xlsx', 
              method = 'curl')
dt <- read_excel('_file.xlsx')
dt2 <- read_excel('_file2.xlsx')
dt 
#' Note that '\$1,000.00' was read as a numerical value! 'read.csv' would not do this.
#' See exercises for a way of cleaning up a numeric variable that was entered with
...
dt2
#'
#' The files `dt` and `dt2` are tibbles with categorical variables represented
...
#' [section below](#regular-expressions-to-replace-strings-within-string-variables)
...
#' 
#' From [\underline{this thread on stackoverflow}](http://stackoverflow.com/questions/31633891/specifying-column-types-when-importing-xlsx-data-to-r-with-package-readxl)
#' here is a function that reads an Excel file and make every variable a character variable
#' to avoid problems with variables that you want to read as characters although the top of
#' the data set contains only values that `read_excel` considers numeric. There is isn't
#' a single argument to `read_excel` to request that all variables be read as characters.
#' Instead, you need to know the number of variable to repeat the `col_types` argument as
#' many times as there are columns. That is, the authors did not build in recycling! This 
#' function first finds out how many variable there are so it can then call `read_excel`
#' with a correct `col_types` argument.
#'
...
#' 
#' SPSS files have long been a problem for R but there is a relatively recent package, 'haven', on
...
#' 
#' __Warning:__ Some functions, e.g. 'lm' may treat a categorical variable as a numeric
...
#' 
#' match(x, table, nomatch) returns the position of each x matched exactly in
#' table
...
seq(1, 5)
seq(1, 5, 0.5)
seq_along(letters) # generates a sequence of indices for a vector or list
#'
...
#'
#' Suppose you have a data frame with many numeric variables recording
#' temperatures in Celsius and you need to transform them to Farenheit
#'
df <- read.csv(text=
'
city,     day1,  day2,  day3
...
xyplot(mathach ~ ses | school, hs, strip = FALSE) + layer(panel.lmline(...))
...
head(hs)
car::some(hs)  # random selection of rows (from car)
#'
...
#' 3. merge with email
#'
merge(grade_report, email, by = 'student')  # inner join, only students in BOTH files
merge(grade_report, email, by = 'student', all = T)  # outer join, students in EITHER files
merge(grade_report, email, by = 'student', all.x = T)  # all rows in first file
#'
...
#'
grades$course_average <- with(grades, capply(gp, course, mean)) # no weights! why?
#'
#' List of transcripts
#'
...
#' Generic function:
#'
to_farenheit <- function(x,...) UseMethod('to_farenheit')
#'
...
#'
#' The above illustrate creating a 'generic function' 
#' and 'methods' for existing
#' classes: here 'numeric', 'integer', 'default' and 'data.frame'
#' 
#' If an object has a class attribute, e.g. data.frames, then the value of the
#' attribute is its class.
#' 
#' If it doesn't have a class attribute then it has an implicit class depending
#' on its type and structure. For example, matrices has the class "matrix" whether
#' their content is numeric or character. However, for a vector, class is 'integer'
#' for an integer, 'numeric' for a double, 'character' for a character. 
#' 
...
#' 
#' 
#' A longitudinal example This is a simple example from a medical
#' study in which each# subject is seen a# on a varying number of
#' visits.# This is the data set in __long__ form.
##' ## Long form ----
#' 
