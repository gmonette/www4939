...
# 
# I use the uniform quantile plot as a 'data dictionary' when
# I'm exploring or analyzing a data set. It tells me much more than
# a numerical summary. 
# 
...
# - With an approximately normal distribution we expect mean -/+ std. dev.
#   to be at approx the 32nd and 68th percentiles.
#
# Things to look for:
# 
# 1. Wild outliers. Often, researchers will use a numerical code, like 99 or 999,
#    fpr missing values but don't exclude them from their analyses so they
#    just enter the analysis as meaningless, and usually very harmful, data.
# 2. The class of variables. Are there character variables that should
#    be converted to factors before an analysis.
# 3. How skewed or kurtotic are distributions.
# 4. The number of levels and the distribution of categorical variables.
#    Are frequencies well balanced or are there rare categories that will
#    present a problem for regressions?
# 5. For categorical variables, does the 'reference level', which appears 
#    as the lowest bar on the barplot, have a reasonable number of observations
#    in order to make comparisons meaningful in regression output?
# 6. Are there ceiling or floor effects, where many values of a numerical
#    variable are at the maximum or minimum for the variable? This shows as
#    a horizontal bar at the top right or at the bottom left of the quantile
#    plot.
# 7. Are there important gaps in numerical values (which will show as 
#    discontinuities in the quantile plot)?
# 8. Are there clumps of values shared by many individuals (which will show
#    as horizontal segments in thw quantile plots)?
# 9. Etc. What would you add? Post it to Piazza with the title: "Quantile plot ideas" 
#
# Note: A function like xqplot is useful for exploratory analysis by a 
#       statistician. I don't think it can play any role in presentations
#       to a general audience. It only perplexes and confuses them.
#       If you want to describe a distribution for a presentation, you should
#       use histograms or densities for numerical data and barcharts for
#       or pie charts for categorical data. Include generous explanations and 
#       interpretations in your captions.
...
#  Intraclass correlation using ranks -- useful to get a quick idea 
#  
gicc(hsfull, ~ school )
...
# We will use a (pre-)randomly selected half of the
...
#
# Note: Where does R find objects?
# 
# If you type a name from the console, R looks:
# 
# 1. in the global environment ".GlobalEnv": objects you've created in the
#    current session or objects that you created previously and loaded as
#    part of the global environment when you start. 
#    Many people consider it a good practice to always start fresh 
#    by executing:
#              rm(list = ls())
#    This is an excellent practice to improve reproducibility.
#    BUT: not that rendering (e.g. with Ctrl-Shift-K) automatically does
#    this so if you compile your report, it won't depend on objects you've
#    left lying around. 
# 2. At visible objects on your search path which you can see with:
# 
search() 
#
#    These are base packages and packages you have loaded. You can also
#    'attach' data frames to your search path with 'attach' (not recommended).
#    It is much better to use 'with' because this will look first in the
#    data frame. If you use 'attach', R will first look in the global environment
#    and you may not get the correct value for the object. Debugging these
#    kinds of problems can cost days of your time. 
#    
...
# 
# For numeric variables, the scatterplot matrix is useful
#
is_num <- sapply(dd, is.numeric) 
is_num
scatterplotMatrix(dd[,is_num], ellipse = TRUE)  # too dense
scatterplotMatrix(dd[,is_num], ellipse = TRUE, pch = '.')  # better?
#
# When working fast, I prefer to build up the process with pipes
dd %>% 
  Filter(is.numeric, . )  %>%  # by default, the pipe feeds its output as the 1st argument
                               # of the function following the pipe,
                               # if that should be another argument, you use a '.'
  scatterplotMatrix(ellipse = T, pch = '.')
#
# Note: Find out about the 'Higher-Order' functions: Filter, Map, Reduce
# Also review higher-order functions: apply, lapply, sapply, capply
# 
# Parenthetical note: 
#   
#   If you are finding these examples slow on you computer
#   don't hesitate to use mtor.sci.yorku.ca:8787. 
# 
#   In addition to being quite fast, you can, for many computations, take
#   advantage of parallel processing.
# 
#   If you are doing some work with 'lapply', you can do it on mtor by
#   using mclapply in the parallel package, which does not work in Windows.
# 
#   See: https://nceas.github.io/oss-lessons/parallel-computing-in-r/parallel-computing-in-r.html
#   and https://dept.stat.lsa.umich.edu/~jerrick/courses/stat701/notes/parallel.html
# 
# Including categorical variables is a bit trickier. 
# There is a meaningful concept like correlation for categorical
# variables: canonical correlation, which you would have seen in
# multivariate analysis. 
# 
# It would be nice to have a function like 'scatterplotMatrix' that would
# show an ellipse capturing the size of the canonical correlation in panels
# where one or both variables of the pair is categorical.  
# 
# Can you find one on the web? Or write one?
# There's a very unsatisfactory function called 'splom'. 
# It treats categorical variables as numerical by using their factor codes,
# which can be highly misleading. 
# However, if a categorical variable has two levels then it's okay. 
# 
# Note the position of the data ellipse for variables with two levels
# where the number of observations is relatively similar in the two levels.
# 
# Can you explain why they look as they do?
# 
splom(dd) +
  layer(panel.dell(..., col = 'red'))
splom(dd, pch = 16, alpha = .05) +
  layer(panel.dell(..., col = 'red', lwd = 2, alpha = 1))
#
# Note that overplotting for level-2 variables eliminates the effect of transparency
# 
### QUESTIONS 2: looking at bivariate relationships ------------------------------
#
...
# Which pairs have the 'strongest' bivariate relationship?
#
#
...
#
#  Variables that are invariant within schools
#  
...
spm(ddu, ell = T)   # between school variables, 'spm' is a shortcut for scatterplotMatrix
#
...
dda <- up (dd, ~school, all = T)   # Note categorical variables get modal value which is generally not useful
dim(dda)
xqplot(dda)
scatterplotMatrix(dda, ell = T)
...
spm( ddca, ell = T  )
#
# Also 'up' has a recently added -- but not fully debugged -- argument 'agg'
# for 'aggregate'
# 
dag <- up(dd, ~ school, agg = ~ Sex + Sector)
some(dag)
dag %>% splom + layer(panel.dell(..., col = 'red'))
# or better, now that we have numerical representations for categorical variable
dd %>% 
  up(~ school, agg = ~ Sex + Sector) %>% 
  Filter(is.numeric, .) %>% 
  spm(ellipse = TRUE)
dd %>% 
  up(~ school, agg = ~ Sex + Sector) %>% 
  Filter(is.numeric, .) %>% 
  splom(.) + layer(panel.dell(..., col = 'red'))
# 
# Note that splom generated warning while 'panel.dell' did not.
# 
# Reason: panel.dell uses a superior method to generate ellipses
# based on the SVD, while spm uses a Cholesky decomposition that 
# spews errors when the ellipse is 'singular' i.e. collapses into
# a line.
# 
# For more on the SVD: https://www.youtube.com/watch?v=JEYLfIVvR9I   
#
### QUESTION 3 ------------------------------------------------------------------                                
# 
# - Some pairs of variables are perfectly correlated. Why?
...
# that capture the phenomena you want to study. 
# 
# These variable often don't come with the raw data! You've got to use
# your knowledge of the subject matter, your discussions with experts in
# the subject matter, and, more importantly, you own creativity and intelligence.
# 
# Examples:
#  
#  Sample size:
...
dd$n <- with(dd, capply(school, school, length))

# Question: what is the advantage of using width instead of attach(dd)
...
dd$ses.iqr <- with(dd, capply(ses, school, IQR, na.rm = FALSE))

# but we could modify the anomymous function it to create other measures of variability:


dd$ses.trange <- # trange for 'truncated range'
  with(dd,
       capply( 
         ses, 
         school,  
...
# also consider measures of variation like sd.


dd %>% 
  within(
    {
      ses.sd <- capply(ses, school, sd, na.rm = TRUE)
    }
  )-> dd

# This illustrates the use '->' instead of '<-'
# It makes the pipe flow from left to right, and top to bottom.
#
some(dd)
head(dd)
...
dd <- within(
  dd,
  {
    ses.rank <- capply(ses, school, rank, na.last = 'keep')
    n <- capply(school, school, length)
    ses.z_rank <- qnorm(ses.rank/(n+1))    # bell curving ses within each each school
    ses.scale <- capply(ses, school, scale)   # z-score within each each school
  }
)
some(dd)
head(dd)
...
# EXERCISE:
#
# Try other possibilities. If you find something, post it on Piazza
#
...
dd$id <- with(dd, reorder(id, ses))    # or

dd$id.sec <- with(dd, reorder(id, ses + 1000* (Sector == "Catholic")))
...
xyplot( mathach ~ ses | id, dd,  layout = c(9,9),
        panel = function( x,y ,..., type) {
          panel.xyplot( x, y, ... )
          panel.lines( dell( x, y, radius = 1:2), type = 'l', col = 'red', ...)
          panel.lmline( x, y, col = 'red', ...)
          panel.lines( lowess( x, y), col = 'black', lwd = 2, ..., type = 'l')
        })
...
# Center point and fitted lines within each group
...
beta <- expand.grid( id = levels( dd$id ))  # OR with(dd, pred.grid(id))
...
#
#         mathach ~ ses  in different Sectors
#
#   Level 1 model:
#
#         mathach ~ 1 + ses
...
# If this fails to converge, the first remedy to try is to
...
#
##  What can we do if the model still does not converge ---------------------------
#

#  We will revisit this as we need it:
#
#  Summary:
...
#     random model is more complex than optimizing algorithm can handle
#     i.e. the variability
...
#     e.g.. it's a flat pancake, or a line or a point. 
#     It does not define an ellipsoid with non-empty interior.
#
...
#       Is the between-school effect of ses different
...
## Creating a model with contextual variable(s) ---------------------------------
...
# 
# pred.grid is in the current version of spida2
# 
pred <- with(hs, pred.grid(Sector, ses.mean = c(-1,0,1), ses.dev = c(-1,1,.1)))
#
# pred.grid creates a Cartesian product of its arguments
# 
# To add other variables that are function of the 'base' variables
# generated in the Cartesian product, we need to add them
# with merge or by explicit definitions
#
pred <- within(
  pred,
  {
    ses <- ses.mean + ses.dev
    id <- paste0(Sector, " mean ses: ", ses.mean)
  }
)
#
...
#     
#  3) Produce some plots that you could use to explain your results.
#
...
#  3 ) Create an L matrix to estimate the
